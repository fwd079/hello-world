...
1. **Dialogues/Forms** To setup permission(s) in Serenity's `xyzDialog.ts` file, we call unitily methods from GeneralLibrary utiltiy class:
...
2. **Grids/Lists** To setup permission(s) in Serenity's `xyzGrid.ts` file, we call unitily methods from GeneralLibrary utiltiy class within Serenity's `createToolbarExtensions():void` method (or any other method where this.toolbar is an object and not `undefined` value) as below:
